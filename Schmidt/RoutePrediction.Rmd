...
# Finding out which positions other than WR are running routes 
```{r}
...
```{r}
plays_of_route_runners |> 
  ggplot(aes(x = inMotionAtBallSnap, fill = inMotionAtBallSnap)) +
  geom_bar() +
  theme_bw() +
  theme(axis.text.x = element_text(size = 8.5, angle = 90), legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold", size = 14), axis.title = element_text(face = "bold", size = 12), axis.text = element_text(face = "bold")) +
  labs(
    title = "Distribution of Players In Motion At Snap",
    x = "Motion At Snap",
    y = "Number of Players"
  )
