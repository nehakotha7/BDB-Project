...
# 323 plays where messed up side is unsure
...
fixed_one_receiver_play <- messed_up_left |> 
...
total_2_left_1_right_1_position1 <- fixing_receivers(messed_up_left, 2, 1, 1) |> 
  filter(player_number == 1,
         official_position %in% c("WR")) |> 
    distinct(gameId, playId) 

# these are the left receivers in those plays
fix_one_receiver_left_part1 <- fixing_receivers(messed_up_left, 2, 1, 1) |> 
  filter(player_number == 1,
         official_position %in% c("WR")) |> 
...
non_route_was_extra <- too_many_receiver_plays |> 
  filter(frameType == "SNAP",
         receiver_in_left_alignment == TRUE | receiver_in_right_alignment == TRUE,
         !is.na(routeRan)) |> 
...
#fixed_too_many_receievers_part1 <- 
  
too_many_receiver_plays |> 
  inner_join(non_route_was_extra) |> 
  filter(frameType == "SNAP", (receiver_in_left_alignment == TRUE | receiver_in_right_alignment == TRUE) & is.na(routeRan)) |> 
  count(official_position) 



  mutate(receiver_in_left_alignment = ifelse(receiver_in_left_alignment == TRUE & is.na(routeRan), FALSE, receiver_in_left_alignment),
         receiver_in_right_alignment = ifelse(receiver_in_right_alignment == TRUE & is.na(routeRan), FALSE, receiver_in_right_alignment))
```
