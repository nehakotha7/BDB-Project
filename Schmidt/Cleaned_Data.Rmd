...
```{r}
# Fixed the play where there was one receiver in alignment
finished_one_receiver_play <- messed_up_left |> 
  filter(total_receivers_in_alignment == 1, 
         official_position == "WR") |> 
  mutate(receiver_in_left_alignment = ifelse(official_position == "WR", TRUE, FALSE),
         receiver_in_right_alignment = FALSE,
...
total_2_left_1_right_1 <- fixing_receivers(messed_up_left, 2, 1, 1)

# there are 27 plays with 1 receiver on each side
...
total_two_left_one_plays_fixed_part1 <- messed_up_left |> 
...
fixed_left_side <- messed_up_left |>
