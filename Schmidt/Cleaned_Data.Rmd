...
# identifying the receivers in left alignment
one_receiver_in_alignment_receivers_left <- too_many_receiver_plays |> 
...
# fixed the plays where the one receiver is in left alignment 
finished_total_one_receiver_left <- too_many_receiver_plays |> 
  inner_join(one_receiver_in_alignment) |> 
  left_join(one_receiver_in_alignment_receivers_left) |> 
...
# identifying the receivers in right alignment
one_receiver_in_alignment_receivers_right <- too_many_receiver_plays |> 
  inner_join(one_receiver_in_alignment) |> 
         filter(frameType == "SNAP",
                player_number == 11,
         official_position %in% c("WR", "TE")) |> 
...
# fixed the plays where the one receiver is in right alignment
finished_total_one_receiver_right <- too_many_receiver_plays |> 
  inner_join(one_receiver_in_alignment) |> 
  left_join(one_receiver_in_alignment_receivers_right) |> 
...
# 23 of them have WR in the 1 position
...
# Sixteen plays that are completely fixed
...
#### Fixing the 4 plays that did not have WRs in the 1 position
...
fixed_left_side <- too_many_receiver_plays |>
  ...
  anti_join(total_2_left_1_right_1_position1) |> 
  left_join(fix_remaining_two_total_one_left_one_right) |> 
  mutate(receiver_in_left_alignment = ifelse(is.na(receiver_in_left_alignment_new), FALSE, TRUE),
         num_receivers_left = 1) |> 
  select(- receiver_in_left_alignment_new) 
