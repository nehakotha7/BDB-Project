...
```{r}
# Players Positions - Include only position of each player, their name, and their NFLId
all_positions <- players |> 
  select(nflId, displayName, position)

# Adds name of player and their position to plays we are predicting 
predicting_plays <- inner_join(predicting_plays, all_positions)
...
#predicting_plays <- predicting_plays |> 
  mutate(set_at_line = ifelse(!is.na(motionSinceLineset), "Yes", "No"))
```

# Update WR Data ----------------------------------

```{r}
plays_of_WR <- predicting_plays |> 
  filter(position == "WR")
```
...
```{r}
predicting_plays |> 
  count(position, inMotionAtBallSnap, motionSinceLineset)
```
