...
receiver_alignment <- function(num_week) {
  num_week |> 
    mutate(receiver_in_left_alignment = ifelse(player_number %in% c(1, 2, 3) & (!is.na(routeRan) | official_position %in% c("TE", "WR")) & playDirection == "left" & frameType == "SNAP", TRUE, FALSE),
           receiver_in_left_alignment = ifelse(player_number %in% c(9, 10, 11) & (!is.na(routeRan) | official_position %in% c("TE", "WR")) & playDirection == "right" & frameType == "SNAP", TRUE, receiver_in_left_alignment),
         receiver_in_right_alignment = ifelse(player_number %in% c(9, 10, 11) & (!is.na(routeRan) | official_position %in% c("TE", "WR")) & playDirection == "left" & frameType == "SNAP", TRUE, FALSE),
         receiver_in_right_alignment = ifelse(player_number %in% c(1, 2, 3) & (!is.na(routeRan) | official_position %in% c("TE", "WR")) & playDirection == "right" & frameType == "SNAP", TRUE, receiver_in_right_alignment))
}
...
```{r}
left_players <- players_predicting |>
  filter(frameType == "SNAP",
...
no_left_players <- players_predicting |> 
...
players_predicting <- inner_join(players_predicting, left_players)
```

## Adding Variables for number of receivers right for each play
```{r}
right_players <- players_predicting |>
  filter(frameType == "SNAP",
...
no_right_players <- players_predicting |> 
...
players_predicting <- inner_join(players_predicting, right_players)
```
